...
finalization, and automated CI fixes on bot branches — runs with **zero** approvals. The only runs
that pause for a `dev-psmdb-approvers` click are the ones whose trigger cannot prove write access:
comment/review-driven commands (`review-handle`, `rebase-handle`, `fast-forward`) and CI for
fork/external PRs (`rbe-approve`).
