...
function addExtUsersRun() {
    ...
    db.getSiblingDB('$external')
        .createUser({user: 'exttestro', roles: [{role: 'read', db: 'test'}]});
    exttest++;

    // exttestrw has create and write on test database
    db.getSiblingDB('$external')
        .createUser({user: 'exttestrw', roles: [{role: 'readWrite', db: 'test'}]});
    exttest++;

    // extotherro has read on other database
    db.getSiblingDB('$external')
        .createUser({user: 'extotherro', roles: [{role: 'read', db: 'other'}]});
    extother++;

    // extotherrw has
    db.getSiblingDB('$external')
        .createUser({user: 'extotherrw', roles: [{role: 'readWrite', db: 'other'}]});
    extother++;

    // extbothro has read on test and other database
    db.getSiblingDB('$external').createUser({
        user: 'extbothro',
        roles: [{role: 'read', db: 'test'}, {role: 'read', db: 'other'}]
    });
    exttest++;
    extother++;
    ...
    db.getSiblingDB('$external').createUser({
        user: 'extbothrw',
        roles: [{role: 'readWrite', db: 'test'}, {role: 'readWrite', db: 'other'}]
    });
    exttest++;
    extother++;
    ...
    db.getSiblingDB('$external').createUser({
        ...
        roles: [{role: 'read', db: 'test'}, {role: 'readWrite', db: 'other'}]
    });
    exttest++;
    extother++;
    ...
    db.getSiblingDB('admin')
        .system.users.find({'$and': [{user: /ext.*/}, {roles: {'$elemMatch': {db: 'test'}}}]})
        .forEach(function(u) {
            print("user: " + u.user);
            findtest++;
        });
    ...
    var findother = 0;
    db.getSiblingDB('admin')
        .system.users.find({'$and': [{user: /ext.*/}, {roles: {'$elemMatch': {db: 'other'}}}]})
        .forEach(function(u) {
            print("user: " + u.user);
            findother++;
        });
