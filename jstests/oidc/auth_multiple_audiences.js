...
const idp_config = {
    token: [
        // Positive cases
        {payload: {sub: "user1", aud: "audience1", claim1: ["group1"]}},
        {payload: {sub: "user2", aud: "audience2", claim2: ["group2"]}},
        {payload: {sub: "user3", aud: "audience3", claim3: ["group3"]}},
        // Negative cases
        {
            payload: {
                sub: "user2",
...
function test_multiple_audiences_for_single_issuer(clusterClass) {
    ...
    const expectedLog = (error) => {
        return {msg: "Failed to authenticate", attr: {mechanism: "MONGODB-OIDC", error: error}};
    };
...
test_multiple_audiences_for_single_issuer(StandaloneMongod);
test_multiple_audiences_for_single_issuer(ShardedCluster);
