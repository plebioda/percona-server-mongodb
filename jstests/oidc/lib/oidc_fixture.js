...
export class OIDCFixture {
    ...
    refresh_token() {
        ...
        try {
            const accessToken = this.conn._refreshAccessToken();
            this.conn.auth({
     ...
     * @param {string[]} roles The expected roles.
     */
    assert_authenticated(user, roles) {
        ...
        if (roles) {
            ...
            for (var i = 0; i < roles.length; i++) {
                assert.eq(authInfo.authenticatedUserRoles[i].role, roles[i], "Role mismatch");
                assert.eq(authInfo.authenticatedUserRoles[i].db, "admin", "Database mismatch for role");
            }
        }
    }
}
