...
export class OIDCIdPMock {
     ...
     *
     * This function starts the server with provided configuration and issuer URL.
     ...
     * @param {string} scopes Scopes requested for the token
     */
    assert_token_requested(client_id, scopes) {
        // NOTE:
        // The '/device/authorize' is used by device authorization flow.
        ...
        var token_search = "/token";
        if (client_id) {
            token_search += ".*client_id=" + client_id + ".*";
        }

        if (scopes) {
            var scopes_search = ".*scope="
            for (const scope of scopes) {
                scopes_search += scope + ".*";
            }
            // TODO: uncomment when implemented on server side
