...
test.assert_authenticated(conn, "idp/user1", ["idp/group1"]);
test.logout(conn);

// authenticate as user2 and check the roles from claim2
test.auth(conn, "user2");
test.assert_authenticated(conn, "idp/user2", ["idp/group2"]);
test.logout(conn);
...
assert(test.checkLogExists(expectedLog("BadValue: InvalidJWT: authorizationClaim 'claim2' is missing")), "Expected log not found for token with invalid claim");
...
assert(test.checkLogExists(expectedLog("BadValue: InvalidJWT: authorizationClaim 'claim1' is missing")), "Expected log not found for token with invalid claim");
