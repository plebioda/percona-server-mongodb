...
void validate(const OidcIdentityProvidersServerParameter& param) {
    ...
    for (std::size_t i = 0; i < param._data.size(); ++i) {
    ...
    for (std::size_t i = 0; i < matchPatternIndexes.size(); ++i) {
        uassert(77705,
                errorMsgHeader(std::ranges::iota_view(i, matchPatternIndexes[i]))
...
void paramDeserialize(OidcIdentityProvidersServerParameter& param, const BSONArray& arr) {
    for (std::size_t i = 0; const BSONElement& elem : arr) {
