...
ExitCode _initAndListen(ServiceContext* serviceContext, int listenPort) {
    ...
    // Cannot use the ServiceContext::ConstructorActionRegisterer to set construct the
    // OidcIdentityProvidersRegistry becasue the PeriodicRunner is not yet initialized when
    // the initializer is executed. Instead, call a static init method which will construct the
    // registry and register it with the ServiceContext.
    OidcIdentityProvidersRegistry::init(serviceContext);
